...
(new message% [parent ACCUEIL]
              [label "Etudiant: DANSOU Koffi Junior"]
              [font policeC]
              )
                  ...
                  [callback  (lambda (o event)
                                 (let
                                     ([rep (message-box/custom 
                                            ...
                                            "oui" "Non" "Annuler" )])
                                     (cond ((eq? rep 1) (send ACCUEIL on-exit))
                                           ))
                                   )]
                  ))
                            ...
                            [callback (lambda (button event)
                                 (let
                                     ([rep (message-box/custom
                                            ...
                                            "oui" "Non" "Annuler")])
                                     (cond ((eq? rep 1) (send Expression set-value "")
                                                       (send Textsimple set-value "")
                                                       (send Expression focus))
                                         )))]
                            ))
...
(send Texte_accueil set-label "APPLICATION DE SIMPLIFICATION D'EXPRESSION")
(sleep/yield 5)
